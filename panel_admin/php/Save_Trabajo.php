...
//---Ruta para enviar el archivo--//
    $ruta="C:/xampp/htdocs/Repository/trabajos";
    $rutas=[
        $ruta,
        $ruta.'/'.$facultad,
...
//---Enviar archivo pdf---//
       ...
       foreach($rutas as $valor){
        if(!file_exists($valor)){
            if(!mkdir($valor,0777)){
                echo "error al crear el directorio :".$valor;
                exit();
            }
        }
...
//-- Guardamos trabajo --//
    $guardar = conexion();
    $guardar=$guardar->prepare("INSERT INTO trabajo(trabajo_titulo,trabajo_resumen,trabajo_año,trabajo_periodo,trabajo_nota_grado,trabajo_link,autor_id,usuario_id) VALUES(:titulo,:resumen,:anio,:periodo,:nota,:link,:autor,:usuario)");
    $marcadores=[
      ":titulo"=>$titulo,
      ":resumen"=>$resumen,
      ":anio"=>$anio,
...
//-- Guardamos los tutores x trabajo --//
    ...
    echo "Trabajo subido exitosamente";

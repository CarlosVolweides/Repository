<?php
	...
	function paginadora_trabajos_admin($pagina,$nregistros,$query,$lq=null,$ctipo=null,$cfacultad=null,$ccarrera=null,$cargo,$sede){
		...
		//WHILE que crea cada box de registro//
			while($row = $cone->fetch(PDO::FETCH_ASSOC)){
				$tabla .= '<li>
				<div class="trabajo">
					<a target="_blank" rel="noopener noreferrer" href="../trabajo.php?id='.$row['trabajo_id'].'">


                	<div class="listado">
						<div class="imagen-trabajo">
								<img style="width:100%;" src="../img/T.E/'.$row['tipo_trabajo_nombre'].'-'.$row['carrera_nombre'].'.webp" alt="">
						...
						<div class="contenedor-infoTrabajo">
							...
							<div class="resumen">
								...
								<a target="_blank" rel="noopener noreferrer" href="../trabajo.php?id='.$row['trabajo_id'].'" class="btn">Leer más →</a>
							</div>

						</div>
					</div>
						<button class="btn-rojo" onclick="window.location.href=\'php/Delete_trabajo.php?id='.$row['trabajo_id'].'\'"><i class="fa-solid fa-trash"></i></button>
					<button class="btn-amarillo "><i class="fa-solid fa-pen" style="color: #414141;"></i></button>
				</div>
				</li>';
				$count++;
			}
		...
		//Concatenar botones de paginacion//
			...
			for($i=1;$i<=$total_paginas;$i++){
				...
				//Nombre de pagina para el ContentID textual
				if($ctipo != null){
					switch($ctipo){
						case 1:
							$tipo='pregrado';
				...
				if($lq == null){
					$tabla.= '<a type="button" '.$clase.' onclick=loadContent("'.$tipo.'",'.$i.','.$cargo.','.$sede.') >'.$i.'</a>
						';
				}else{
					$tabla.= '<a type="button" '.$clase.' onclick=loadContent("buscar",'.$i.','.$cargo.','.$sede.') >'.$i.'</a>
						';
				}
			}
			$tabla.= '</div>';
